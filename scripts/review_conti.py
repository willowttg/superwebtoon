...
3. `assets/memes-used.md` — 밈이 이전 편과 겹치는지 본다.
4. 검토 대상: `{conti}` (EP.01~06은 이전 포맷이라 예시·비교 대상이 아니다. 읽지 않는다.)

검토 관점 (기획 규칙의 항목을 하나씩 대조하되, 규칙에 없어도 독자로서 걸리는 것은 적는다):
- 특징 5개가 각각 "이거 너잖아" 하고 친구에게 보낼 만큼 구체적인가, 아니면 유형 밈 수준인가. 서로 겹치는 특징이 없는가.
- 특징 문장을 가리고 장면 열만 봐도 웃긴가. 과장 장치가 없는 컷은 삽화로 지적한다.
- 특징 문장이 짧은 음슴체 한 줄인가, 캐릭터 이름·느낌표·이모지가 없는가.
- 대사가 특징 문장을 반복하지 않는가. 그 캐릭터가 그 순간 실제로 할 법한 길이·어투인가. 캐릭터 성격에 맞는가.
- 순서: 보편적인 것이 앞, 설명이 필요한 것이 뒤인가. 정면 구도 3컷 초과·연속, 먹구름 사용 여부.
- MBTI 노출, 서사로 억지로 잇기 같은 금지 사항.
