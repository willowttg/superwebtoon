...
- 내레이션·속마음은 **1인칭 "나는 …"**. "우리는"·3인칭 설명("이 유형은 …")은 쓰지 않는다. 내레이션은 **컷당 2줄 고정**(긴 줄은 합성 시 자동 축소). **내레이션에 캐릭터 이름을 넣지 않는다**(친구·엄마·동생 등 관계로 지칭; 이름은 대사·CTA에서만). CTA만 독자에게 말한다. 어미는 문어체·구어체를 섞어 리듬을 준다("…한다!", "…않을래.").
