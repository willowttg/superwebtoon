...
- **구성 = 표지 1 + 특징 컷 5 = 6장.** 표지: 제목(초능력 이름) + **제목의 초능력을 한 장으로 과장한 코믹 그림** — 그림만 봐도 제목이 읽히고 공감이 가야 한다. 특징 컷: 번호 + 특징 한 줄 + 그 특징을 한 단계 과장한 그림 1장. 컷끼리 서사로 잇지 않고 나열한다. 순서는 가장 보편적으로 찔리는 것부터, 설명이 필요한 것은 뒤로.
- **제목** = 그 유형의 통용되는 단점을 능력처럼 바꾼 후킹 카피. 형식 `[…] 초능력자 특징`. 예: 「No를 Yes로 바꿔 말하는 초능력자 특징」. 핵심 구절만 포인트 컬러 볼드.
- **특징 문장**은 음슴체로, 상황과 행동을 구체적이고 맛깔나게 (`전화는 죽어도 안 받고 끊어지면 톡으로 답함`, `배달 기사님 엘리베이터 타실 때까지 숨어서 훔쳐 봄`). 유형 이름 같은 추상 문구(`오프라인 쇼핑 공포증 있음`) 금지. 슬라이드에서 두 줄까지 허용. 캐릭터 이름 금지(할머니·동생·친구 등 관계어), 느낌표·이모지 금지. MBTI는 작품 어디에도 쓰지 않는다.
...
- **초능력 소품**: 콘티 단계에서 그 편의 초능력과 밀접한 소품 1개를 정한다. 초능력 아이템처럼 보이도록 색·모양이 독특하거나 화려해야 한다(칙칙한 천·망토류 금지). 두부 시트를 참조로 소품을 착용한 시트 `episodes/epNN/ref/dubu-<소품>-sheet.png`를 **medium**, 1536×1024로 생성하고(프롬프트는 `dubu-sheet-prompt.txt` 구조 + 소품 design 블록), 표지와 소품이 등장하는 컷은 이 시트를 두부 시트 다음 참조로 넣고 프롬프트에 소품 블록을 덧붙인다. 콘티 표에 `소품: ○○`를 적는다.
- 표지: 소품을 착용한 두부가 제목의 초능력을 발휘하는 장면(예: 「집에서만 충전」 → 배터리 배낭 케이블을 콘센트에 꽂고 충전 중). 두부가 그냥 서 있는 표정 컷은 표지가 아니다. 특징 컷은 감정 기호(땀방울·눈물·낙서 소용돌이·세로 그늘선·하트·반짝이) 자유. **먹구름은 쓰지 않는다.**
...
- [ ] 초능력 소품 시트가 있고 표지·소품 컷이 그 시트를 참조했는가
