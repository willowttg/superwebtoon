...
- **콘티 검증 (1회, 프롬프트 작성 전).** 콘티를 다 쓴 뒤 장면·내레이션·대사 세 열을 **각각 따로** 1→7로 이어 읽는다.
  - 내레이션만: 연결어(하지만·사실·그런데·…지만)가 실제 반전을 담는가. 단점은 능력의 반대가 아니라 같은 능력이 잘못된 자리에서 발동한 것이므로, 3→4가 반대 관계가 아닌데 역접으로 잇거나 3 끝 `…지만`과 4 첫머리 `하지만`이 겹치면 억지 역접이다.
  - 대사만: 내레이션과 같은 정보를 반복하지 않는가, 그 컷의 펀치를 대사가 맡는가.
