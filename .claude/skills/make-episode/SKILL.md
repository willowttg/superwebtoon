---
name: make-episode
description: 「작은 힘에도 작은 책임은 따른다」 에피소드 1편 제작 — MBTI 유형의 단점을 초능력으로 재해석한 7컷 캐러셀. 콘티 → 컷 이미지 생성 → 텍스트 합성 → 프리뷰 → 캡션. "에피소드 만들어", "EP.02", "INTJ 편", "콘티 짜줘", "컷 생성" 등에 사용.
---
...
| 2–6 | 흐름만 고정: **장점(능력이 남에게 좋게 작용, 조연이 수혜자) → 단점 첫 등장 → 단점 심화**. 컷 배분은 에피소드마다 정한다. 장점은 보통 1–2컷이면 충분하다 — 두 번 보여주려면 두 번째는 첫 번째의 결과여야 한다(같은 능력을 다른 상대에게 한 번 더는 안 됨). 상황 컷과 속마음 컷의 순서도 자유. | **단점 첫 등장 컷 = 공유 유도 컷.** 가장 구체적이고 찔리는 상황, "이거 너잖아" 하고 친구에게 보내게 되는 컷. 여기에 가장 공을 들인다 |
| 7 | 마무리 = **저장 유도 문장** + 태그 CTA | "그래도 그 능력 덕분에 누군가는…" 한 줄 위로 → "당신 옆의 초능력자를 태그해 주세요" |

톤 전환: 장점 구간 밝음 → 단점 구간 저채도 → 7 망토 + 포인트 컬러 복귀.

- **기본 톤은 코믹.** 공감은 웃음 뒤에 따라오게 한다 — 단점은 처량하게가 아니라 **과장·자폭 개그**로 그린다(상황을 한 단계 더 밀어붙이기, 속마음의 자기 변명·궤변, 조연의 무표정 리액션·한마디 태클). 컷마다 웃음 포인트가 하나씩 있는지 보고, 진지한 문장은 7컷 위로 한 줄에만 허용한다. 단점 구간의 마지막 컷도 우울로 마감하지 않고 한 번 더 자폭으로 끝낸다(EP.06 6컷 창가 먹구름 X).

- **한 편 = 한 사건.** 능력을 상대·장소만 바꿔 여러 번 보여주는 나열(EP.05 1–3: 행인·친구·동생에게 각각 사과)은 금지. 한 상황이 시간순으로 굴러가고 앞 컷의 결과가 다음 컷의 원인이 되게 한다(EP.04 주문 못 함 → 가족 배고픔 → 음식 나온 뒤 후회, EP.05 지각 → 그날 밤 → 다음 날). 장소·상대를 바꾸는 컷은 앞 컷과 이어지는 끈을 하나 남긴다 — 같은 소품이 따라오기(EP.06 서핑보드 4→5→7), 앞 컷 구도의 반전(EP.04 1↔4), 시간 라벨 `-그날 밤-`, 7컷에서 앞 대사 되돌리기(EP.06 클라이밍 초크백).
- **컷마다 새 정보 하나.** 이미 말한 능력·단점을 문장만 바꿔 되풀이하지 않는다(EP.04 2·3 "메뉴판 구석까지 읽는다" 두 번). 단점 전환도 `사실…` 내레이션이 아니라 사건(누가 늦음, 택배 도착)으로 연다.

- **콘티 검증 (1회, 프롬프트 작성 전).** 콘티를 다 쓴 뒤 장면·내레이션·대사 세 열을 **각각 따로** 1→7로 이어 읽는다.
  - 내레이션만: 컷마다 한 줄로 요약해 같은 요약이 둘이면 하나를 지우거나 다음 단계로 밀어붙인다. 연결어(하지만·사실·그런데·…지만)가 실제 반전을 담는가. 단점은 능력의 반대가 아니라 같은 능력이 잘못된 자리에서 발동한 것이므로, 3→4가 반대 관계가 아닌데 역접으로 잇거나 3 끝 `…지만`과 4 첫머리 `하지만`이 겹치면 억지 역접이다. 소리 내 읽었을 때 사람이 쓰는 말인가. **AI 말투 금지** — 아래 패턴이 보이면 그 줄을 다시 쓴다:
    ...
    대신 **그 순간 눈에 보이는 것·입에서 나오는 말**로 쓴다 (예: "막상 나가면 즐겁겠지만 시동 거는 게 너무 힘든 걸 어떡하냐고!"). 7컷 위로도 정리·격언이 아니라 상대에게 던지는 말 한마디로.
...
- 초능력 발동(1·장점 컷·7): 에피소드 소품 + 반짝이. 소품은 `assets/props.md`에서 기존 편과 종류·색이 겹치지 않게 정하고 콘티 전에 표에 추가한다. 색은 크림색 몸과 대비되게 **화려해도 좋다**(EP.01 연두+보라 선글라스). 착용 위치를 프롬프트에 명시(선글라스는 눈을 덮게). **소품은 매 컷 넣지 않고 장면에 필요한 컷에만** 쓰며, 콘티 상단에 쓰는 컷 번호를 적는다. 7컷 망토 `#C77B3F`는 공통. 단점 컷: 감정 기호. **감정 기호(먹구름·땀방울·낙서 소용돌이·세로 그늘선·눈물·하트·반짝이 등)는 한 에피소드 안에서 컷마다 다른 것을 쓰고 중복시키지 않는다** — 콘티의 속마음 열에 `감정: ○○`로 컷별 배정을 적고 프롬프트에 그 하나만 넣는다. 먹구름은 편당 1컷까지.
...
- [ ] 단점 첫 등장 컷이 친구에게 보낼 만큼 구체적인가 / 컷마다 웃음 포인트가 있는가 / 7컷이 저장할 만한 한 문장인가
