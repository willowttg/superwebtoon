...
- **소품** = 에피소드마다 그 초능력을 상징하는 소품 1개를 정한다 (예: 마음 읽기 → 헤드폰). 보리가 착용하는 형태여야 하며, 표지와 능력 발동 컷에서 착용한다.
- **표지 + 7컷 = 8장** (인스타 상한).

| 컷 | 역할 | 비고 |
|---|---|---|
| 표지 | 제목 + 부제 + 보리 (망토 + 소품, POWER ACTIVATED 포즈) | 제목의 핵심 단어 1개만 포인트 컬러 |
...
- **소품 시트**: 컷 생성 전에 소품을 착용한 보리 캐릭터 시트 `img/bori-prop.png`를 먼저 만든다 (기본 시트를 참조로, 같은 턴어라운드·표정 구성, `high`). 이후 소품 착용 컷과 표지는 첫 참조 이미지를 기본 시트 대신 `img/bori-prop.png`로 바꾸고, 프롬프트 헤더에 소품 설명 1줄을 추가한다. 미착용 컷은 기본 시트 그대로.
...
```
episodes/epNN/
  conti.md        제목·부제·소품·컷별 [장면 / 소품 착용 여부 / 내레이션 / 속마음 / 프롬프트 요점]
  prompts/bori-prop.txt, cutN.txt
  img/bori-prop.png, cutN.png, slideN.png
  compose.html
